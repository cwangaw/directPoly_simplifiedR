...
double PolyElement::minDistFromEdgeMidPt() {
  double min_dist = edgePtr(0) -> length();
  for (int i = 0; i < num_vertices; i++) {
    Point mid_point = (*vertexPtr(i) + *vertexPtr( (i-1+num_vertices) % num_vertices )) / 2;
    if (edgePtr(i+1)->lambda(mid_point) < min_dist) {
      min_dist = edgePtr(i+1)->lambda(mid_point);
    }
    if (edgePtr((i-1+num_vertices) % num_vertices)->lambda(mid_point) < min_dist) {
      min_dist = edgePtr( (i-1+num_vertices) % num_vertices )->lambda(mid_point);
    }
  }
  return min_dist;
}

std::vector<std::vector<Point>> PolyElement::partitionLines() {
  std::vector<std::vector<Point>> lines;
  lines.clear();

  // The following line is for the partition corresponding to midpoints of the edges 
  ...
  for (int i = 0; i < num_vertices; i++) {
    ...
    // The following block is for the partition corresponding to midpoints of the edges 
...
std::vector<std::vector<Point>> PolyElement::partition() {
  ...
  // Construct edges
  for (int i = 0; i < num_vertices; i++) {
    std::vector<Point> this_edge{*vertexPtr((i-1+num_vertices) % num_vertices), *vertexPtr(i)};
  ...
  // We process the lines list considering triangles (need no partition)
  // and quadrilaterals (need partition only for non-parallel opposite edges)

  if ( num_vertices == 3 ) {
    lines.clear();
  } else if ( num_vertices == 4 ) {
    // test parallel edges
    bool isParallel_02 = 0;
    bool isParallel_13 = 0;

    if (fabs(edgePtr(0)->tangent() * edgePtr(2)->normal()) < polyelement_eps) {
      isParallel_02 = 1;
    }

    if (fabs(edgePtr(1)->tangent() * edgePtr(3)->normal()) < polyelement_eps) {
      isParallel_13 = 1;
    }

    if (isParallel_02 || isParallel_13) {
      std::vector<std::vector<Point>> new_lines;
      std::vector<Tensor1> new_lines_normal;
      std::vector<Tensor1> new_lines_tangent;
      new_lines.clear();
      new_lines_normal.clear();
      new_lines_tangent.clear();

      if (!isParallel_02) {
        new_lines.push_back(lines[0]);
        new_lines_normal.push_back(lines_normal[0]);
        new_lines_tangent.push_back(lines_tangent[0]);
        new_lines.push_back(lines[2]);
        new_lines_normal.push_back(lines_normal[2]);
        new_lines_tangent.push_back(lines_tangent[2]);
      }

      if (!isParallel_13) {
        new_lines.push_back(lines[1]);
        new_lines_normal.push_back(lines_normal[1]);
        new_lines_tangent.push_back(lines_tangent[1]);
        new_lines.push_back(lines[3]);
        new_lines_normal.push_back(lines_normal[3]);
        new_lines_tangent.push_back(lines_tangent[3]);
      }

      lines = new_lines;
      lines_normal = new_lines_normal;
      lines_tangent = new_lines_tangent;
    }
  }
  ...
  /*
    for (int i = 0; i < edge_segs.size(); i++) {
    std::cout << "edge segs of e_" << i << std::endl;
    for (int j = 0; j < edge_segs[i].size(); j++) {
      std::cout << edge_segs[i][j] << " " << std::endl;
    }
    std::cout << std::endl;
    }

    for (int i = 0; i < line_segs.size(); i++) {
    std::cout << "line segs of l_" << i << std::endl;
    for (int j = 0; j < line_segs[i].size(); j++) {
      std::cout << line_segs[i][j] << " " << std::endl;
    }
    std::cout << std::endl;
    }
  */
